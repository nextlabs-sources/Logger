...
CUSTOM_SCRIPT_DIR="/var/opt/nextlabs/logger/shared/conf"

# Check if customers have their own script. If they do, run their script
if [[ -f $CUSTOM_SCRIPT_DIR/custom_script_logger.sh ]]
then
  sh $CUSTOM_SCRIPT_DIR/custom_script_logger.sh
fi
...
if [ "$1" = 'run' ]; then
  ...
  ##
  [ -f /var/opt/nextlabs/logger/shared/tomcat/conf/server.xml ] && \
